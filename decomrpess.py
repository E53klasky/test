...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Validate compressed ADIOS2 files against original"
    )
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Suppress detailed progress messages"
    )
    parser.add_argument(
        "--output",
        "-o",
...
def main():
    ...
    for eb_idx, eb in enumerate(error_bounds):
        ...
        compressed_file = compressed_base.replace(
            ".bp", f"_compressed_{op_name}_eb_{eb_str}.bp"
        )
        ...
        try:
            ...
            r_orig = ReaderClass.Reader(
                unique_reader_orig, original_file, xml=xml, comm=comm
            )
            r_comp = ReaderClass.Reader(
                unique_reader_comp, compressed_file, xml=xml, comm=comm
            )
            ...
            while True:
                ...
                if (
                    status_orig != adios2.bindings.StepStatus.OK
                    or status_comp != adios2.bindings.StepStatus.OK
                ):
                    break
                ...
                for var_name, var_info in available_vars.items():
                    ...
                    if var_type in ["double", "float"]:
                        ...
                        if not quiet:
                            ...
                            print(
                                f"    {status_symbol} {var_name}: NRMSE={nrmse:.6e}, PSNR={psnr:.2f} dB"
                            )
                        ...
                        if not passed:
                            all_vars_pass = False
                            failed_vars.append(
                                f"{var_name} (step {current_step}): NRMSE={nrmse:.6e} > {eb}"
                            )
            ...
            if var_results:
                ...
                avg_psnr = np.mean(
                    [v["psnr"] for v in var_results if np.isfinite(v["psnr"])]
                )
            ...
            # Add timing info
            time_msg = f"\n🕐 *** {op_name.upper()} DECOMPRESSION/VALIDATION TIME (EB={eb}): {eb_elapsed:.2f} seconds ***"
            print(time_msg)
    ...
    print(f"\n{'#'*60}")
    print(f"### TIMING SUMMARY - {op_name.upper()} DECOMPRESSION/VALIDATION ###")
    print(f"{'#'*60}")
    ...
    print(
        f"\n🕐 *** TOTAL {op_name.upper()} DECOMPRESSION/VALIDATION TIME: {total_elapsed:.2f} seconds ***"
    )
    print(f"{'#'*60}\n")

    # Add timing summary to results file
    results.append("")
    results.append("#" * 80)
    results.append(
        f"### TIMING SUMMARY - {op_name.upper()} DECOMPRESSION/VALIDATION ###"
    )
    results.append("#" * 80)
    for result in timing_results:
        results.append(
